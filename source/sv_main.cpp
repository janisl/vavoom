...
class VLightningThinker : public VThinker
{
	DECLARE_CLASS(VLightningThinker, VThinker, 0)
	NO_DEFAULT_CONSTRUCTOR(VLightningThinker)

private:
	enum
	{
		LIGHTNING_OUTDOOR	= 197,
	...
	int				LightningFlash;
	TArray<int>		LightningLightLevels;
public:
...
IMPLEMENT_CLASS(V, LightningThinker)
...
void SV_SpawnServer(const char *mapname, bool spawn_thinkers, bool titlemap)
{
	...
	if (spawn_thinkers)
	{
		...
		if (GLevelInfo->LevelInfoFlags & VLevelInfo::LIF_Lightning)
		{
			VLightningThinker* Lightning = (VLightningThinker*)GLevel->SpawnThinker(
				VLightningThinker::StaticClass());
			Lightning->Init();
		}
	}
...
//==========================================================================
//
//	VLightningThinker::Init
//
//==========================================================================

void VLightningThinker::Init()
{
	guard(VLightningThinker::Init);
	if (Initialised)
	{
		return;
	}
	...
	if (secCount)
	{
		LightningLightLevels.SetNum(secCount);
...
//==========================================================================
//
//	VLightningThinker::Tick
//
//==========================================================================

void VLightningThinker::Tick(float DeltaTime)
{
	guard(VLightningThinker::Tick);
	//	Update lightning
...
//==========================================================================
//
//	VLightningThinker::DoLightningFlash
//
//==========================================================================

void VLightningThinker::DoLightningFlash()
{
	guard(VLightningThinker::DoLightningFlash);
	int 		i;
	sector_t 	*tempSec;
	int 		*tempLight;
	bool	 	foundSec;
	...
	if (LightningFlash)
	{
		...
		if (LightningFlash)
		{
			tempLight = LightningLightLevels.Ptr();
	...
	if (foundSec)
	{
		...
		StartSound(TVec(0, 0, 0), 0, GSoundManager->GetSoundID("world/thunder"),
			0, 1, 0, false);
	}
...
//==========================================================================
//
//	VLightningThinker::ForceLightning
//
//==========================================================================

void VLightningThinker::ForceLightning()
{
	guard(VLightningThinker::ForceLightning);
...
void VLevelInfo::ForceLightning()
{
	...
	VLightningThinker* Lightning = NULL;
	TThinkerIterator<VLightningThinker> It(XLevel);
	if (It)
	{
		Lightning = *It;
	}
	if (!Lightning)
	{
		Lightning = (VLightningThinker*)XLevel->SpawnThinker(
			VLightningThinker::StaticClass());
		Lightning->Init();
	}
