...
{
	float xind = (float)((2 * n - 1) * (2 * n - 1));
	for (int i = 0; i < n; i++) 
	{
		float xi = (float)(i + 0.5);
		w[i] = ino((float)(beta * sqrt((double)(1. - 4 * xi * xi / xind)))) / ino((float)beta);
	}
}
...
static void designfir(float* g, float fc)
{
	...
	{
		float xi = (float) (i + 0.5);
		float omega = (float)(PI * xi);
		g[i] = (float)(sin( (double) omega * fc) / omega);
	}
	
	float att = 40.; // attenuation  in  db
	float beta = (float)(exp(log((double)0.58417 * (att - 20.96)) * 0.4) + 0.07886 * (att - 20.96));
