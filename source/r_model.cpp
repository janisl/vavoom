...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawAliasModelAmbient
//
//==========================================================================

bool VAdvancedRenderLevel::DrawAliasModelAmbient(const TVec& Org, const TAVec& Angles,
	float ScaleX, float ScaleY, VModel* Mdl, int Frame, int NextFrame,
	int Version, vuint32 Light, float Inter, bool Interpolate)
{
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawAliasModelTextures
//
//==========================================================================

bool VAdvancedRenderLevel::DrawAliasModelTextures(const TVec& Org, const TAVec& Angles,
	float ScaleX, float ScaleY, VModel* Mdl, int Frame, int NextFrame,
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawAliasModelLight
//
//==========================================================================

bool VAdvancedRenderLevel::DrawAliasModelLight(const TVec& Org, const TAVec& Angles,
	float ScaleX, float ScaleY, VModel* Mdl, int Frame, int NextFrame,
	int Version, float Inter, bool Interpolate)
{
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawAliasModelFog
//
//==========================================================================

bool VAdvancedRenderLevel::DrawAliasModelFog(const TVec& Org, const TAVec& Angles,
	float ScaleX, float ScaleY, VModel* Mdl, int Frame, int NextFrame,
	int Version, vuint32 Fade, float Inter, bool Interpolate)
{
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawEntityModelAmbient
//
//==========================================================================

bool VAdvancedRenderLevel::DrawEntityModelAmbient(VEntity* Ent, vuint32 Light,
	float Inter)
{
	guard(VAdvancedRenderLevel::DrawEntityModelAmbient);
	VState* DispState = (Ent->EntityFlags & VEntity::EF_UseDispState) ?
		Ent->DispState : Ent->State;
	bool Interpolate;
	// Check if we want to interpolate model frames
	...
	if (Ent->EntityFlags & VEntity::EF_FixedModel)
	{
		...
		return DrawAliasModelAmbient(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, Mdl,
			DispState->InClassIndex,
			DispState->NextState ? DispState->NextState->InClassIndex :
			DispState->InClassIndex, Ent->ModelVersion, Light, Inter,
			Interpolate);
	}
	else
	{
		return DrawAliasModelAmbient(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, DispState,
			DispState->NextState ? DispState->NextState : DispState,
			Ent->ModelVersion, Light, Inter, Interpolate);
	}
	unguard;
}
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawEntityModelTextures
//
//==========================================================================

bool VAdvancedRenderLevel::DrawEntityModelTextures(VEntity* Ent, float Inter)
{
	guard(VAdvancedRenderLevel::DrawEntityModelTextures);
	VState* DispState = (Ent->EntityFlags & VEntity::EF_UseDispState) ?
		Ent->DispState : Ent->State;
	bool Interpolate;
	// Check if we want to interpolate model frames
	...
	if (Ent->EntityFlags & VEntity::EF_FixedModel)
	{
		...
		return DrawAliasModelTextures(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, Mdl,
			DispState->InClassIndex,
			DispState->NextState ? DispState->NextState->InClassIndex :
			DispState->InClassIndex, GetTranslation(Ent->Translation),
	...
	else
	{
		return DrawAliasModelTextures(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, DispState,
			DispState->NextState ? DispState->NextState : DispState,
			GetTranslation(Ent->Translation), Ent->ModelVersion, Inter, Interpolate);
	}
	unguard;
}
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawEntityModelLight
//
//==========================================================================

bool VAdvancedRenderLevel::DrawEntityModelLight(VEntity* Ent, float Inter)
{
	guard(VAdvancedRenderLevel::DrawEntityModelLight);
	VState* DispState = (Ent->EntityFlags & VEntity::EF_UseDispState) ?
		Ent->DispState : Ent->State;
	bool Interpolate;
	// Check if we want to interpolate model frames
...
//==========================================================================
//
//	VAdvancedRenderLevel::DrawEntityModelFog
//
//==========================================================================

bool VAdvancedRenderLevel::DrawEntityModelFog(VEntity* Ent, vuint32 Fade,
	float Inter)
{
	guard(VAdvancedRenderLevel::DrawEntityModelFog);
	VState* DispState = (Ent->EntityFlags & VEntity::EF_UseDispState) ?
		Ent->DispState : Ent->State;
	bool Interpolate;
	// Check if we want to interpolate model frames
	...
	if (Ent->EntityFlags & VEntity::EF_FixedModel)
	{
		...
		return DrawAliasModelFog(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, Mdl,
			DispState->InClassIndex,
			DispState->NextState ? DispState->NextState->InClassIndex :
			DispState->InClassIndex, Ent->ModelVersion, Fade, Inter, Interpolate);
	}
	else
	{
		return DrawAliasModelFog(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			Ent->Angles, Ent->ScaleX, Ent->ScaleY, DispState,
			DispState->NextState ? DispState->NextState : DispState,
			Ent->ModelVersion, Fade, Inter, Interpolate);
	}
	unguard;
}
