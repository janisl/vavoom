...
static void DrawModel(VLevel* Level, const TVec& Org, const TAVec& Angles,
	...
	const char* Skin, int Version, vuint32 Light, vuint32 Fade, float Alpha,
	bool Additive, bool IsViewModel, float Inter)
{
	guard(DrawModel);
	...
	bool IsViewModel, float Inter)
{
	...
	DrawModel(Level, Org, Angles, ScaleX, ScaleY, *SMdl->DefaultClass, FIdx,
		Skin, Version, Light, Fade, Alpha, Additive, IsViewModel, Inter);
	return true;
	unguard;
}
...
bool VRenderLevel::DrawAliasModel(const TVec& Org, const TAVec& Angles,
	float ScaleX, float ScaleY, VState* State, const char* Skin, int Version,
	...
	float Inter)
{
	...
	DrawModel(Level, Org, Angles, ScaleX, ScaleY, *Cls, FIdx, Skin, Version,
	...
	float Alpha, bool Additive, float Inter)
{
	...
	if (Ent->EntityFlags & VEntity::EF_FixedModel)
	{
		...
		return DrawAliasModel(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			...
			Ent->State->InClassIndex, *Ent->ModelSkin, Ent->ModelVersion,
	...
	else
	{
		return DrawAliasModel(Ent->Origin - TVec(0, 0, Ent->FloorClip),
			...
			*Ent->ModelSkin, Ent->ModelVersion, Light, Fade, Alpha,
			Additive, false, Inter);
	}
	unguard;
}
	...
	int Frame, const char* Skin, float Inter)
{
	...
	DrawModel(NULL, Origin, Angles, 1.0, 1.0, *SMdl->DefaultClass, FIdx,
		Skin, 0, 0xffffffff, 0, 1.0, false, false, 0);
	...
	float Angle)
{
	...
	DrawModel(NULL, Origin, Angles, 1.0, 1.0, *Cls, FIdx, "", 0, 0xffffffff,
		0, 1.0, false, false, 0);
