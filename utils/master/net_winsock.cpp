...
public:
	...
	const char* AddrToString(sockaddr_t*);
	int StringToAddr(const char*, sockaddr_t*);
	int GetSocketAddr(int, sockaddr_t*);
	const char* GetNameFromAddr(sockaddr_t*);
...
int VWinSockDriver::Init()
{
	...
	{
		printf("Winsock TCP/IP Initialisation failed.\n");
		if (--winsock_initialised == 0)
			WSACleanup();
		return -1;
	}
	...
	{
		printf("WINS_Init: Unable to open control socket\n");
		if (--winsock_initialised == 0)
			WSACleanup();
		return -1;
	}
	...
	((sockaddr_in *)&broadcastaddr)->sin_port = htons((vuint16)HostPort);

	printf("Winsock TCP/IP Initialised\n");
...
int VWinSockDriver::Broadcast(int socket, vuint8* buf, int len)
{
	...
	if (socket != net_broadcastsocket)
	{
		if (net_broadcastsocket != 0)
			Error("Attempted to use multiple broadcasts sockets\n");
		...
		{
			printf("Unable to make socket broadcast capable\n");
			return -1;
		}
...
const char* VWinSockDriver::AddrToString(sockaddr_t* addr)
{
...
const char* VWinSockDriver::GetNameFromAddr(sockaddr_t* addr)
{
...
int VWinSockDriver::GetAddrFromName(const char *name, sockaddr_t *addr)
{
	...
	addr->sa_family = AF_INET;
	((sockaddr_in *)addr)->sin_port = htons((vuint16)HostPort);
