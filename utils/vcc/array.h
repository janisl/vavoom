...
protected:
	void Realloc(int ElementSize)
	{
		if (ArrayMax)
		{
			if (Data)
			{
				Data = VavoomUtils::Realloc(Data, ArrayMax * ElementSize);
			}
			else
			{
				Data = Malloc(ArrayMax * ElementSize);
			}
		}
		else
		{
...
public:
	...
	TArray(void) : FArray()
	{}
	TArray(int InNum) : FArray(InNum, sizeof(T))
	{}
	TArray(const TArray& Other) : FArray(Other.ArrayNum, sizeof(T))
	{
		ArrayNum = 0;
